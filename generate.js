...
function stringify(obj, indent = 2) {
  if (Array.isArray(obj)) {
    ...
    let out = "{\n";
    keys.forEach((k, i) => {
      let keyStr = k.includes(' ') || k.includes('-') ? "'" + k + "'" : k;
    ...
    // avoid issues with literal newlines in the generated file by properly escaping them or by saving literally
    let cleanStr = obj.replace(/\\n/g, ' ').replace(/'/g, "\\'");
    return "'" + cleanStr + "'";
  }
  return String(obj);
}
