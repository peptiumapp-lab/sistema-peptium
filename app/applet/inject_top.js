...
const peptideBlocks = code.split(/(?=\n\s*{\s*\n\s*id:\s*['"])/);
...
for (let i = 0; i < peptideBlocks.length; i++) {
  ...
  if (idMatch) {
    const id = idMatch[1];
    if (deepBenefits[id]) {
      const newB = deepBenefits[id].map(b => `\n      '${b.replace(/'/g, "\\'")}'`).join(',') + '\n    ';
      const replacement = `\n    clinicalBenefits: [${newB}],`;
      
      if (block.includes('clinicalBenefits:')) {
         block = block.replace(/(clinicalBenefits:\s*\[)([^\]]*)(\])/, `$1${newB}$3`);
      } else if (block.includes('clinicalEfficacy:')) {
         block = block.replace(/(clinicalEfficacy:\s*\[)([^\]]*)(\])/, `clinicalBenefits: [${newB}]`);
      } else {
         block = block.replace(/(features:\s*\[[^\]]*\],)/, `$1${replacement}`);
      }
...
fs.writeFileSync(filepath, peptideBlocks.join(''));
console.log('Injected deep benefits for top peptides!');
